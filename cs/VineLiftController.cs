...
///   ✅ 只有在 Trigger 里按下按键，才会播放声音（平台照常按键就能动）
/// </summary>
public class VineLiftController : MonoBehaviour
{
    ...
    [Header("音效设置（可选）")]
    public AudioClip liftSfx;          // 按键触发时播放的声音（玩家在 trigger 里才会播）
    [Range(0f, 1f)]
    public float sfxVolume = 1f;
    public bool sfxPlayOnce = false;   // 只在第一次触发时播放一遍
    ...
    private bool _sfxPlayedOnce = false;
    ...
    {
        // 测试用按键：
        // 现在逻辑是：按键一定会 ToggleDirection（保持你原来的习惯）
        // 但只有玩家在 Trigger 里，才会额外播放声音
    ...
    {
        if (_audio == null || liftSfx == null)
            return;

        if (sfxPlayOnce && _sfxPlayedOnce)
            return;

        _audio.volume = sfxVolume;
        _audio.PlayOneShot(liftSfx, sfxVolume);
        _sfxPlayedOnce = true;
    }
